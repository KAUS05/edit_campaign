...
document.querySelector('.custom-select-wrapper-sender').addEventListener('click', function() {
  this.querySelector('.custom-select-sender').classList.toggle('open-sender');
})

for (const option of document.querySelectorAll(".custom-option-sender")) {
option.addEventListener('click', function() {
    if (!this.classList.contains('selected-sender')) {
        this.parentNode.querySelector('.custom-option-sender.selected-sender').classList.remove('selected-sender');
        this.classList.add('selected-sender');
        this.closest('.custom-select-sender').querySelector('.custom-select__trigger-sender span').textContent = this.textContent;
    }
})
}
